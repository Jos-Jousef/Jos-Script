...
client.on("message" , async msg => {
 ...
 if(afk){
  ...
  if(msg.author.id === kisi){
       ...
       msg.channel.send(new Discord.MessageEmbed().setColor('BLACK').setDescription(`<@${kisi}> Artık Afk Değilsiniz`))
...
client.on('channelCreate', async channel => {
  const c = channelguild.channels.cache.find(db.fetch(`codeminglog_${channel.guild.id}`));
  if (!c) return;
...
client.on('channelDelete', async channel => {
  const c = member.guild.channels.cache.find(db.fetch(`codeminglog_${channel.guild.id}`));
  if (!c) return;
...
client.on('emojiCreate', emoji => {
  const c = emoji.guild.channels.cache.find(db.fetch(`codeminglog_${emoji.guild.id}`));
  if (!c) return;
...
client.on('emojiUpdate', (oldEmoji, newEmoji) => {
  const c = newemoji.guild.channels.cache.find(db.fetch(`codeminglog_${newEmoji.guild.id}`));
  if (!c) return;
...
client.on('guildBanAdd', async (guild, user) => {    
    const channel = guild.channels.cache.find(db.fetch(`codeminglog_${guild.id}`));
  if (!channel) return;
...
client.on('messageDelete', async message => {    
  if(message.author.bot) return
    ...
    const channel = messageguild.channels.cache.find(db.fetch(`codeminglog_${message.guild.id}`));
  if (!channel) return;
...
client.on('messageUpdate', async(oldMessage, newMessage) => {
    ...
    const channel = oldMessageguild.channels.cache.find(db.fetch(`codeminglog_${oldMessage.guild.id}`));
    if(!channel) return;
...
client.on('roleCreate', async (role) => {    
    ...
    const channel = roleguild.channels.cache.find(db.fetch(`codeminglog_${role.guild.id}`));
  if (!channel) return;
...
client.on('voiceStateUpdate', (oldMember, newMember) => {
 ...
 // if (!logA[oldMember.guild.id]) return;
  ...
  var kanal = oldMemberguild.channels.cache.find(db.fetch(`codeminglog_${oldMember.guild.id}`).replace("<#", "").replace(">", ""))
  if (!kanal) return;
